...
const addProductToCart = async (req, res) => {
	...
	try {
		...
		if (product.length == 0) {
			...
			throw new Error("product unavailable now");
		}
		...
		const productIsFoundInCart = await Carts.findAll({
			where: { product_id: data.product_id, user_id: data.user_id },
		});

		if (productIsFoundInCart.length != 0) {
			let updatedQuantity =
				productIsFoundInCart[0].quantity + data.quantity;
			await Carts.update(
				{
			...
			res.status(200).send({
				status: httpStatusText.SUCCESS,
				data: null,
				msg: "quantity of products updated",
			});
		} else {
...
const removeAllProductsFromCart = async (req, res) => {
	...
	try {
		...
		res.status(200).send({
			status: httpStatusText.SUCCESS,
			data: null,
			msg: "user cart removed successfully",
		});
	} catch (error) {
		res.status(400).send({
			status: httpStatusText.FAIL,
